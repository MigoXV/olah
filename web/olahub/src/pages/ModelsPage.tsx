...
const ModelsPage = () => {
  ...
  const fetchModels = async () => {
    try {
      ...
      if (response.data.models && response.data.datasets.length > 0) {
  ...
  return (
    <div>
      <div className="mb-6">
        <Title level={2}>模型库</Title>
        <Paragraph className="text-gray-600">浏览和下载各种 AI 模型，加速您的开发流程</Paragraph>
      </div>
      ...
      {loading ? (
              ...
              <List.Item>
                <Card hoverable className="h-full flex flex-col">
                  <div className="flex-1">
                    <Link to={`/models/${model.id}`} className="text-lg font-medium text-blue-600 hover:text-blue-800">
                      {model.name}
                    </Link>
        ...
        <Empty description="没有找到符合条件的模型" image={Empty.PRESENTED_IMAGE_SIMPLE} className="py-12" />
      )}
    </div>
  )
}
